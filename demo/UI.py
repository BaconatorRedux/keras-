            ...
            if Lic_pred:
                ...
                for i, lic_pred in enumerate(Lic_pred):
                    if i == 0:
                        ...
                        self.can_pred1.create_text(35, 15, text=lic_pred[1], anchor='nw', font=('宋体', 28))
                    elif i == 1:
                        self.lic_Tk2 = ImageTk.PhotoImage(Image.fromarray(lic_pred[0][:, :, ::-1]))
                        self.can_lic2.create_image(5, 5, image=self.lic_Tk2, anchor='nw')
                        self.can_pred2.create_text(40, 15, text=lic_pred[1], anchor='nw', font=('宋体', 28))
                    elif i == 2:
                        self.lic_Tk3 = ImageTk.PhotoImage(Image.fromarray(lic_pred[0][:, :, ::-1]))
                        self.can_lic3.create_image(5, 5, image=self.lic_Tk3, anchor='nw')
                        self.can_pred3.create_text(40, 15, text=lic_pred[1], anchor='nw', font=('宋体', 28))

            else:  # Lic_pred为空说明未能识别
                self.can_pred1.create_text(47, 15, text='未能识别', anchor='nw', font=('宋体', 27))
